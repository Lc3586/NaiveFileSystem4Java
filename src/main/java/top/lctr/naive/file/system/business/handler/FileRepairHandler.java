...
public class FileRepairHandler
    ...
    private void startCheck(Object state) {
        try {
            //将待修复的任务添加至队列
            fileService.getUnrepairedIdList(false)
                       .forEach(x -> super.addTask(x,
                                                   false));

            super.handler();
        } catch (Exception ex) {
            logger.warn("添加待修复的文件至队列失败",
                        ex);
        }
...
//                    continue;
    ...
    private Tuple2<Boolean, CommonFile> tryGetFileInfo(String id,
        ...
        CommonFile file = fileService.get(id,
                                          withTransactional);
        if (file == null) {
            logger.warn(String.format("修复文件信息失败, 指定的文件信息不存在[ID: %s]",
                                      id));
        ...
        if (file.exists()) {
            ...
            if (fileInfo.getStorageType()
                ...
                String path = Paths.get(fileService.getWWWRootDirectory(),
                                        relativePath)
                                   .toString();
                Tuple2<InputStream, Integer> image = ImageHelper.getBase64Image(fileInfo.getPath());
...
//            fileInfo.setState(FileState.已删除);
            logger.warn(String.format("修复文件信息错误，文件[ID: %s]不存在，等待下次修复",
                                      fileInfo.getId()));
    ...
    private boolean completed(CommonFile fileInfo,
        ...
        try {
            ...
            logger.warn(String.format("更新个人文件信息失败, 文件信息[ID: %s]",
                                      fileInfo.getId()),
                        ex);
            return false;
        }
